...
def choose_this_children_nyero():
    ...
    for key in gyerek_szotar.keys():
        if len(gyerek_szotar[key]) == 0:
            ...
            lista = []
            for i in range(len(gyerek_szotar[key])):
                #print(aranyok[str(gyerek_szotar[key][i])][1]) #todo az a baj, hogy nem stringben voltak elmentve az erteke, hanem listben..
...
if __name__ == "__main__":
    ...
    with open("nyero_proba.json", "w") as write_file:
